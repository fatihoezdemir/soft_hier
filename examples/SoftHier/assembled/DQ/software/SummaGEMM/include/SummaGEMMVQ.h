#ifndef _SUMMA_GEMMVQ_H_
#define _SUMMA_GEMMVQ_H_
...
void SummaGEMMRunVQ(SummaGEMMInfo* info) {
    ...
    if (info->cluster_active) {
            ...
            for (int n = 0; n < info->N_iter; ++n) {
                ...
                if (flex_is_dm_core()) {
                    if (info->cluster_for_rowwise == 1) {
                        ...
                        flex_dma_async_2d(info->L1_X1,                            /*destination*/
...
#else
                        flex_dma_async_2d(info->L1_X1, /*destination*/
                                          info->X_tile_base + m * info->X_tile_M_iter_offset +
                                              n * info->X_tile_N_iter_offset, /*source*/
                                          info->K_tile * DATA_TYPE_BYTE,      /*transfer size*/
                                          info->K_tile * DATA_TYPE_BYTE,      /*destination stride*/
                        ...
                        if (info->summa_group_x > 1) {
                            flex_dma_async_broadcast(info->L1_X1 /*dst_offset*/, info->L1_X1 /*src_offset*/,
                    ...
                    if (info->cluster_for_colwise == 1) {
                        ...
                        flex_dma_async_2d(info->L1_W1, /*destination*/
                                          info->W_tile_base + m * info->W_tile_M_iter_offset +
                                              n * info->W_tile_N_iter_offset, /*source*/
                        ...
                        if (info->summa_group_y > 1) {
                            flex_dma_async_broadcast(info->L1_W1 /*dst_offset*/, info->L1_W1 /*src_offset*/,
                    ...
                    if (flex_is_dm_core()) {
                        if (k < info->K_iter) {
                            if (info->cluster_for_rowwise == 1) {
                                // load X from west edge
#if GEMM_RESHA_X_FROM_ENABLE == 1
                                uint64_t origin_elem_offest =
                                    (info->X_tile_base_offset + m * info->X_tile_M_iter_offset +
...
#endif
                                uint64_t mapped_offset = (mapped_m * GEMM_RESHAPE_X_FROM_K + mapped_k) * DATA_TYPE_BYTE;
                                flex_dma_async_2d(DMA_L1_X,                               /*destination*/
                                                  info->X_tile_base + mapped_offset,      /*source*/
                                                  info->K_tile * DATA_TYPE_BYTE,          /*transfer size*/
                                                  info->K_tile * DATA_TYPE_BYTE,          /*destination stride*/
                                                  GEMM_RESHAPE_X_FROM_K * DATA_TYPE_BYTE, /*source stride*/
                                ...
                                flex_dma_async_2d(DMA_L1_X, /*destination*/
                                                  info->X_tile_base + m * info->X_tile_M_iter_offset +
                                                      n * info->X_tile_N_iter_offset +
                                                      k * info->X_tile_K_iter_offset, /*source*/
                                                  info->K_tile * DATA_TYPE_BYTE,      /*transfer size*/
                                                  info->K_tile * DATA_TYPE_BYTE,      /*destination stride*/
                                                  info->K_size * DATA_TYPE_BYTE,      /*source stride*/
                                                  info->M_tile /*repeat*/);           // Start 2D iDMA
                                ...
                                // row-wise multicast
                                if (info->summa_group_x > 1) {
                                    flex_dma_async_broadcast(DMA_L1_X /*dst_offset*/, DMA_L1_X /*src_offset*/,
                                                             info->L1_X_size /*transfer_size*/,
                                                             info->group.wakeup_row_mask /*row_mask*/,
                                                             (ARCH_NUM_CLUSTER_Y - 1) /*col_mask*/);
                                    flex_dma_async_wait_all(); // Wait for iDMA Finishing
                            ...
                            if (info->cluster_for_colwise == 1) {
                                // load W from south edge
                                flex_dma_async_2d(DMA_L1_W, /*destination*/
                                                  info->W_tile_base + m * info->W_tile_M_iter_offset +
                                                      n * info->W_tile_N_iter_offset +
                                ...
                                // col-wise multicast
                                if (info->summa_group_y > 1) {
                                    flex_dma_async_broadcast(DMA_L1_W /*dst_offset*/, DMA_L1_W /*src_offset*/,
                                                             info->L1_W_size /*transfer_size*/,
                                                             (ARCH_NUM_CLUSTER_X - 1) /*row_mask*/,
                                                             info->group.wakeup_col_mask /*col_mask*/);
                                    flex_dma_async_wait_all(); // Wait for iDMA Finishing
                        ...
                        if (info->store_recorded == 1 && info->store_active == 1) {
                            ...
                            if (((info->cluster_in_group_id_x >= bid && info->cluster_in_group_id_x < eid) &&
                                ...
                                if (info->group_reduction == 1) {
                                    // Reduce Z
                                    flex_dma_async_reduction(DMA_L1_Z /*dst_offset*/, DMA_L1_Z /*src_offset*/,
                                ...
                                uint64_t origin_elem_offest =
                                    (info->Z_tile_base_offset + info->store_m * info->Z_tile_M_iter_offset +
                                     info->store_n * info->Z_tile_N_iter_offset) /
                                    DATA_TYPE_BYTE;
                                ...
                                flex_dma_async_2d(info->Z_tile_base + mapped_offset,    /*destination*/
                                                  DMA_L1_Z,                             /*source*/
                                                  info->N_tile * DATA_TYPE_BYTE,        /*transfer size*/
                                                  ...
                                                  info->M_tile /*repeat*/);             // Start 2D iDMA
#else
                                flex_dma_async_2d(info->Z_tile_base + info->store_m * info->Z_tile_M_iter_offset +
                                                      info->store_n * info->Z_tile_N_iter_offset, /*destination*/
                                                  DMA_L1_Z,                                       /*source*/
                                                  info->N_tile * DATA_TYPE_BYTE,                  /*transfer size*/
        ...
        if (flex_is_dm_core() && info->store_active == 1) {
            if (info->group_reduction == 1) {
                // Reduce Z
                flex_dma_async_reduction(DMA_L1_Z /*dst_offset*/, DMA_L1_Z /*src_offset*/,
                                         info->L1_Z_size /*transfer_size*/, COLLECTIVE_REDSUM_TYPE /*fmt*/,
                                         ~info->group.wakeup_row_mask /*row_mask*/,
                                         ~info->group.wakeup_col_mask /*col_mask*/);
                flex_dma_async_wait_all(); // Wait for iDMA Finishing
            }
            // Store Z
#if GEMM_RESHA_Z_TO_ENABLE == 1
            uint64_t origin_elem_offest = (info->Z_tile_base_offset + info->store_m * info->Z_tile_M_iter_offset +
                                           info->store_n * info->Z_tile_N_iter_offset) /
                                          DATA_TYPE_BYTE;
            uint64_t origin_n = origin_elem_offest % GEMM_N_SIZE;
            uint64_t origin_m = origin_elem_offest / GEMM_N_SIZE;
