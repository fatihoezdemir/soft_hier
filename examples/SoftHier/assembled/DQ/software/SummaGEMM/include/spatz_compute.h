#ifndef SPATZ_COMPUTE_HH
#define SPATZ_COMPUTE_HH
...
// Gustavson-style GEMV with RVV.
// accumulate=true adds into existing C (used across K tiles); accumulate=false initializes C.
static inline void spatz_gemv_fp16_full_legacy(uint16_t* matrix_a, uint16_t* matrix_b, uint16_t* matrix_c,
                                               const uint32_t M, const uint32_t N, const uint32_t P, bool accumulate) {
    uint32_t p   = 0;
    uint32_t avl = P;
    uint32_t vl;
    do {
        ...
        for (uint32_t m = 0; m < M; m++) {
            ...
            if (accumulate) {
                asm volatile("vle16.v v16, (%0)" ::"r"(p_c) : "v16");
            } else {
