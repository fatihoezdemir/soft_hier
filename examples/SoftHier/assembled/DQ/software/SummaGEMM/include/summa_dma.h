...
static inline void summa_vq_load_indices(SummaGEMMInfo* info, uint32_t dst_L1_IDX, int m, int n, int k) {
    ...
    uint32_t single_cb_tile_len = N_tile_compressed * info->K_tile * VQ_IDX_BYTES;

    // Load indices from separate arrays (one per codebook)
    for (int cb = 0; cb < VQ_NUM_CBS; ++cb) {
        ...
        uint32_t dst_offset = dst_L1_IDX + cb * single_cb_tile_len;
    ...
    flex_dma_async_broadcast(dst_L1_IDX, dst_L1_IDX, info->vq.L1_IDX_size, info->group.wakeup_col_mask,
                             (ARCH_NUM_CLUSTER_X - 1));
    flex_dma_async_wait_all();
}

static inline void summa_vq_dequantize_tile(const SummaGEMMInfo* info, uint32_t dst_L1_W, uint32_t src_L1_IDX,
