...
static inline void summa_vq_dequantize_tile_fused(const SummaGEMMInfo* info, uint32_t dst_L1_Z, int buffer_idx,
    ...
    const uint16_t* scales = (const uint16_t*)(uintptr_t)src_L1_Scales;
    const uint16_t* x_ptr  = (const uint16_t*)(uintptr_t)src_L1_x;
    ...
    for (uint32_t k = 0; k < info->K_tile; ++k) {
        const uint8_t* idx_cb0_row = idx_cb0_base + k * row_length;
        const uint8_t* idx_cb1_row = idx_cb1_base + k * row_length;
