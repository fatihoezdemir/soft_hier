...
void spatz_matmul_fp8(uint8_t* matrix_a, uint8_t* matrix_b, uint8_t* matrix_c, const uint32_t M, const uint32_t N,
                      const uint32_t P) {
...
void spatz_matmul_fp16(uint8_t* matrix_a, uint8_t* matrix_b, uint16_t* matrix_c, const uint32_t M, const uint32_t N,
                       const uint32_t P) {
...
void spatz_AspB_matmul_fp16(uint8_t* matrix_a, uint8_t* matrix_b, uint16_t* matrix_c, uint8_t* index_b,
                            const uint32_t M, const uint32_t N, const uint32_t P, const uint8_t spN,
                            const uint8_t spM) {
            ...
            for (uint32_t i = 0; i < N; i++) { // row
                for (uint32_t j = 0; j < ((P * spN / spM) / vl); j++) { // vl block
                    for (uint32_t k = 0; k < vl / spN; k++) {           // nm block
                        for (uint32_t q = 0; q < spN; q++)
                            index_b[q + k * spN + j * vl + i * (P * spN / spM)] += k * spM;
                    }
                }
            }
            pre_vl = vl;
...
void spatz_matmul_unroll2_fp16(uint8_t* matrix_a, uint8_t* matrix_b, uint16_t* matrix_c, const uint32_t M,
...
void spatz_matmul_unroll4_fp16(uint8_t* matrix_a, uint8_t* matrix_b, uint16_t* matrix_c, const uint32_t M,
                               const uint32_t N, const uint32_t P) {
...
void spatz_AspB_matmul_unroll4x2_wxfp16_opt(uint8_t* __restrict matrix_a, uint8_t* __restrict matrix_b,
                                            uint16_t* __restrict matrix_c, uint8_t* __restrict index_b,
                                            const uint32_t M, const uint32_t N, const uint32_t P, const uint8_t spN,
                                            const uint8_t spM) {
