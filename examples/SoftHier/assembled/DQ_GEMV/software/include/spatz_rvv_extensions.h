...
#define ENCODE_OPVFX(funct6, vm, vs2, vs1, frs1_3b, vd)                                                                \
    (((uint32_t)((funct6) & 0x3Fu) << 26) | ((uint32_t)((vm) & 0x01u) << 25) | ((uint32_t)((vs2) & 0x1Fu) << 20) |     \
     ((uint32_t)((vs1) & 0x1Fu) << 15) | ((uint32_t)((frs1_3b) & 0x07u) << 12) | ((uint32_t)((vd) & 0x1Fu) << 7) |     \
     ((uint32_t)OPC_OPVFX))
...
#define EMIT_WORD_IMM(word32_const) asm volatile(".word %0" ::"i"((uint32_t)(word32_const)))

/* Use when any arg is runtime/computed: loads into a temp reg then emits. */
#define EMIT_WORD_REG(word32_val)                                                                                      \
    do {                                                                                                               \
